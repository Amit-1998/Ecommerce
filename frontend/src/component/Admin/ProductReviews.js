...
const ProductReviews = ({ history }) => {
  ...
  useEffect(() => {
        if(productId.length === 24) {
            dispatch(getAllReviews(productId)); // update ka button bhi na dabana pade, sirf productId pura likhte hi uske reviews aa jaye
        }
        if(error) {
            alert.error(error);
        ...
        if(isDeleted) {
            ...
            dispatch({ type: DELETE_REVIEW_RESET }); // isdeleted ko false karne ke liye varna baar baar call hota rahega
        }
